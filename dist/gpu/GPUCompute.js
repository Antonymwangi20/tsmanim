...
export class GPUCompute {
    ...
    queue = null; // GPUQueue | null
    adapter = null; // GPUAdapter | null
    isAvailable = false;
    particleBuffer = null;
    forceBuffer = null;
    computePipeline = null; // GPUComputePipeline | null
    /**
    ...
    async init() {
        try {
            ...
            const gpuAvailable = nav && nav.gpu;
            if (!gpuAvailable) {
     ...
     */
    async createParticleSimulationPipeline(particleCount) {
        if (!this.device)
            return false;
        try {
            const shaderCode = `
     ...
     * Run compute shader
     */
    async compute(deltaTime, forceCount) {
     ...
     */
    async readParticles() {
        if (!this.device || !this.queue || !this.particleBuffer)
            return null;
        try {
            const GPUBufferUsage = {
                COPY_DST: 0x04,
            ...
            const commandEncoder = this.device.createCommandEncoder();
            commandEncoder.copyBufferToBuffer(this.particleBuffer.buffer, 0, stagingBuffer, 0, this.particleBuffer.buffer.size);
            this.queue.submit([commandEncoder.finish()]);
