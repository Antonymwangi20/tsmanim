...
export class AnimationAI {
     ...
     */
    suggestAnimation(node, targetState, style = 'organic') {
        ...
        if (pattern.overshoot > 0 && targetState.position) {
            ...
            const overshootPos = targetState.position.add(targetState.position.sub(node.position).mul(pattern.overshoot));
     ...
     */
    optimizeKeyframes(keyframes, tolerance = 0.01) {
    ...
    pointLineDistance(point, lineStart, lineEnd) {
        ...
        if (dx === 0 && dy === 0)
            return 0;
        const num = Math.abs(dy * point.time -
            dx * point.value +
     ...
     */
    calculateSmoothedPath(start, end, segments) {
        const path = [];
     ...
     */
    analyzeMotion(keyframes) {
        if (keyframes.length < 2)
            return { avgVelocity: 0, peakVelocity: 0, acceleration: 0, isLinear: true };
        let totalDistance = 0;
        let peakVel = 0;
     ...
     */
    suggestEasing(keyframes) {
        const motion = this.analyzeMotion(keyframes);
        if (motion.isLinear)
            return 'linear';
        if (motion.acceleration > 0.5)
            return 'ease-out';
        if (motion.acceleration < -0.3)
            return 'ease-in';
        return 'ease-in-out';
    }
}
export const animationAI = new AnimationAI();
//# sourceMappingURL=AnimationAI.js.map