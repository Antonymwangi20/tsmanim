...
export class WebExporter {
     ...
     */
    exportReact(scene, componentName = 'AnimatedScene') {
...
`;
     ...
     */
    exportVue(scene, componentName = 'AnimatedScene') {
        const animations = this.extractAnimations(scene);
        const nodes = this.extractNodes(scene);
        return `<template>
...
`;
    ...
    generateReactAnimationCode(animations) {
        return animations
            .map((anim, i) => {
            return `    // Animation ${i}
  ...
  if (elapsed >= ${anim.startTime || 0} && elapsed < ${(anim.startTime || 0) + (anim.duration || 1)}) {
    ...
    generateThreeJSObjects(nodes) {
        return nodes
            .map((node, i) => `    const mesh${i} = new THREE.Mesh(
      new THREE.BoxGeometry(1, 1, 1),
    ...
    generateThreeJSAnimationCode(animations) {
        return animations
            .map((anim, i) => {
    ...
    generateCanvasAnimationCode(animations) {
        return `      ctx.fillStyle = '#000000';
      ctx.font = '20px sans-serif';
      ctx.fillText(\`Time: \${elapsed.toFixed(2)}s\`, 20, 30);`;
    }
    generateLottieLayers(scene) {
        return [];
    }
}
