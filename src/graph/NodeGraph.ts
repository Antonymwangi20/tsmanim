...
export class NodeGraph extends EventEmitter {
   ...
   */
  addNode(type: string, position: { x: number; y: number }, label?: string): GraphNode {
  ...
  connect(outputPortId: string, inputPortId: string): boolean {
    ...
    if (outPort.type !== inPort.type && inPort.type !== 'trigger') return false;
  ...
  disconnect(outputPortId: string, inputPortId: string): boolean {
    ...
    if (outIdx >= 0) outPort.connections.splice(outIdx, 1);
    if (inIdx >= 0) inPort.connections.splice(inIdx, 1);
   ...
   */
  private createNodeInstance(type: string, position: { x: number; y: number }, label?: string): GraphNode {
   ...
   */
  private getNodeFactories(): Record<string, (id: string, pos: any, label: string) => GraphNode> {
    return {
      'Constant': (id) => ({
        id,
        type: 'Constant',
