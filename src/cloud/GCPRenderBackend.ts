...
export class GCPRenderBackend {
  ...
  private jobMap: Map<string, GCPRenderJob> = new Map();
...
app.listen(PORT, () => {
  ...
  estimateGCPCosts(
    frameCount: number,
    taskCount: number,
    estimatedFrameTimeMinutes: number
  ): {
    ...
    const cloudRunCompute = 
      (computeSeconds * 4 * 0.00002400) + 
...
export const createGCPRenderBackend = (config: GCPConfig): GCPRenderBackend => {
