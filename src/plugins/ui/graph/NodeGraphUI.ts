...
export class NodeGraphUI {
  ...
  private connectedPorts: Array<{ portId: string; pos: Vector2 }> = [];
   ...
   * Draw node in graph
   */
  private drawNode(node: GraphNode, renderCtx: RenderContext): void {
   ...
   */
  private drawConnections(renderCtx: RenderContext): void {
    const { ctx, scale, offset } = renderCtx;
   ...
   */
  private drawTemporaryConnection(renderCtx: RenderContext): void {
    if (this.connectedPorts.length < 1) return;
   ...
   */
  private getNodeAtPosition(pos: Vector2): string | null {
    for (const node of this.graph.getAllNodes()) {
   ...
   * Export as image
   */
  exportAsImage(filename: string = 'graph.png'): void {
    this.render();
 ...
 */
export const createNodeGraphUI = (graph: NodeGraph, config: UIConfig): NodeGraphUI => {
