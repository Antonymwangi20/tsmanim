...
program
  ...
  .action(async (scriptPath, options) => {
    try {
      ...
      const scene: Scene = scriptModule.default ?? scriptModule.scene;
      ...
      const frames = await renderer.renderScene(
        scene,
        { outputDir: framesDir, frameFormat: 'png' },
        (completed: number, total: number) => {
          ...
          process.stdout.write(
            `\r⏳ ${percent}% (${completed}/${total}) | ${fps.toFixed(1)} fps | ETA: ${eta.toFixed(0)}s`
          );
        }
      );
